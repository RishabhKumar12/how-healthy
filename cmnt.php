...
<?php include "config.php";  ?> 
  ...
  <h2>Welcome
    <?php 
	 echo $_SESSION['usr'];
	$q1=mysql_query('select * from tbl_comments where user_id="'.$_SESSION['d'].'" ');  
	?>
 ...
 <!--h3>  Your comments will be shown after permission being granted by admin</h3>-->  
   ...
   <form  action="addcommentdb.php?tid=<?php echo $row['comment_id'];  ?>" method="post" enctype="multipart/form-data" >
 ...
 <tr><td colspan="2" style="cursor:pointer; text-decoration:underline; color:#0066CC" id="<?php echo $row['comment_id'] ; ?>">Comments <?php echo ("($o)"); ?></td><td></td>
	...
	<tr style="display:none"  id="<?php echo $row['comment_id'].'1'; ?>" ><td></td><td colspan="3">
