...
function show()
{
document.getElementById('cmnt_box').style.display="block";
document.getElementById('child_cmnt').style.display="none";
}
<?php 
...
<?php include "config.php";  ?> 
 ...
 <!--h3>  Your comments will be shown after permission being granted by admin</h3>-->  
   ...
   <?php $q2=mysql_query('select * from tbl_comments where parent_id="'.$row['comment_id'].'" ');   
	...
	$o=mysql_num_rows($q2);?>
 <tr><td colspan="2" style="cursor:pointer; text-decoration:underline; color:#0066CC" id="<?php echo $row['comment_id'] ; ?>">Comments <?php echo ("($o)"); ?></td><td></td>
 <td> <input type="button" value="Comment"  onclick="return show();"/></td>
 </tr>
	
	
	<tr style="display:none;" id="child_cmnt">
	<td colspan="3">
	...
	<tr id="cmnt_box" style="display:none;" > <td></td>
